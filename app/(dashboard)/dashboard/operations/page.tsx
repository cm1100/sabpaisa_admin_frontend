...
import { StyledCard, CentralTitle, CentralText, StyledSpace, CentralBadge, Tag, Empty, CentralProgress, StyledStatistic, CentralProTable, Segmented } from '@/components/ui';
...
export default function DashboardOperationsPage() {
  ...
  const [breakdownTab, setBreakdownTab] = useState<'gateways' | 'types' | 'priority'>('gateways');
  ...
  return (
    <CentralPageContainer withBackground title="Operations — Gateway Sync" extra={headerExtra}>
      {loading && (
      ...
      {sync && (
        <>
          ...
          <ResponsiveRow gutter={16}>
            <ResponsiveCol xs={24}>
              <StyledCard variant="interactive" title="Operations Breakdown" data-testid="dashboard-operations-breakdown">
                <StyledSpace style={{ justifyContent: 'space-between', width: '100%', marginBottom: 8 }}>
                  ...
                  <Segmented
                    ...
                    value={breakdownTab}
                    onChange={(v:any)=>setBreakdownTab(v)}
                  />
                </StyledSpace>

                {breakdownTab === 'gateways' && (
                  <CentralProTable
